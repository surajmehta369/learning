...
if (!$needs_login) {
    ...
    $videos = [];
    $videos_result = mysqli_query($conn, "
    SELECT cv.*, s.full_name AS uploader_name, q.id AS quiz_id
    FROM course_videos cv
    LEFT JOIN signup s ON cv.uploader_id = s.id
    LEFT JOIN quizzes q ON cv.id = q.video_id
    ...
    $note_query = "SELECT note_content FROM video_notes WHERE user_id = ? AND video_id = ?";
    ...
    if ($row = $result->fetch_assoc()) {
        $existing_note = $row['note_content'];
    }
    $stmt->close();
